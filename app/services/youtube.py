...
class YouTubeService:
    ...
    to bypass Cloud/Datacenter IP bans without needing cookies.
    """
    ...
    def fetch_transcript(video_id: str) -> Optional[List[Dict]]:
        ...
        for instance in YouTubeService.PIPED_INSTANCES:
            try:
                ...
                if not subtitles:
                    print(f"⚠️ [YOUTUBE SERVICE] {instance} reported no subtitles. Trying next server...")
                    continue 
                
                # 2. Find the English subtitle track (prioritize manual over auto-generated)
                en_subs = [s for s in subtitles if s.get("code", "").startswith("en") or "English" in s.get("name", "")]
                if not en_subs:
                ...
                # ==========================================
                # 🛑 THE CRITICAL FIX
                # Piped API returns relative URLs (e.g., /subtitles?v=123).
                # We MUST attach the instance domain before downloading!
                # ==========================================
        ...
        for block in blocks:
            ...
            for line in lines:
                ...
                if match:
                    ...
                    # Clean up HTML-like formatting tags (e.g., <c.color>, <i>)
                    clean_line = re.sub(r'<[^>]+>', '', line).strip()
                    if clean_line:
                        text_lines.append(clean_line)
            
            if text_lines and end_sec > 0:
                # VTT files often repeat words for scrolling effect. Remove exact duplicate lines.
